...
womens_categories = {
    'Tops': 'http://www.ae.com/web/browse/category.jsp?catId=cat10049',
    'Bottoms': 'http://www.ae.com/web/browse/category.jsp?catId=cat10051',
    'Accessories': 'http://www.ae.com/web/browse/category.jsp?catId=cat570010',
    'Footwear': 'http://www.ae.com/web/browse/category.jsp?catId=cat90122',
    'Swim': 'http://www.ae.com/web/browse/category.jsp?catId=cat10054'
}
...
def scrape_item(category, gender, link, subcategory):
    ...
    while index < len(colors_links):
        ...
        while count < len(view_links):
            ...
            v = { 'viewId': count + 1 }
            v['viewUrl'] = 'http:' + a_tag.find('img')['src'].split('?')[0] + '?$pdp-406$&wid=1713'
            views.append(v)
            count += 1
        color['views'] = views
        colors.append( color )
    ...
    for category in cats.keys():
        ...
        while index <= count - 2:
            ...
            for link in product_links:
                ...
                prod_url = site_url + link['href']
                print prod_url
                try:
                    scrape_item(category, gender, prod_url, subcategory)
                except Exception, e:
                    log_file.writelines([str(e), prod_url])
            index += 2
            #break
        #break

if __name__ == '__main__':
    scrape_data(mens_categories, 'men')
    scrape_data(womens_categories, 'women')
    log_file.close()
